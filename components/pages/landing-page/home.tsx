...
const Home = () => {
  return (
    <section className="bg-home min-h-screen relative flex justify-center items-center overflow-hidden" id="Home">
      <div className="absolute inset-0 h-full bg-linear-to-b from-white/95 from-20% to-white/60 flex justify-center items-center px-37.5 2xl:px-50">
        <Image src="/bg-home-white.svg" alt="Background Home" width={1626} height={733} className="w-full h-full hidden xl:block" />
      </div>

      <div className="z-10 w-full max-w-5xl flex flex-col items-center gap-6 sm:gap-8 lg:gap-10 px-4 sm:px-6 lg:px-10 py-8">
        <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} transition={{ duration: 0.8, type: 'spring', stiffness: 100 }} className="flex items-center justify-center">
          <Image src="/logo-title.svg" alt="logo-title" width={488} height={234} className="w-full max-w-85 md:max-w-100 lg:max-w-112.5 2xl:max-w-137.5 h-auto px-2" />
        </motion.div>
        ...
        <motion.p
          ...
          Buktikan lah diri anda bahwa anda adalah seorang penggiat teknologi dengan mental yang kuat dalam kompetisi DN Roboco 2026.
        </motion.p>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 md:gap-5 w-full justify-center items-center max-w-md sm:max-w-none">
          <motion.a
            ...
            <p>daftar sekarang</p>
            <Image src="/arrow-left.svg" alt="Arrow Icon" width={20} height={20} className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />
          </motion.a>
