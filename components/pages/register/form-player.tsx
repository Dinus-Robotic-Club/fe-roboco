        ...
        <div key={index} className="flex flex-col gap-6">
          ...
          <Field label={`NAMA PEMAIN ${index + 1}`} error={errors[index]?.participantsName}>
            <input
              ...
              className={`p-4 bg-white rounded-xs shadow-md border-2 ${errors[index]?.participantsName ? 'border-red-500' : 'border-gray-200'}`}
          ...
          <Field label={`PERAN PEMAIN ${index + 1}`} error={errors[index]?.participantsRoleInTeam}>
            <select
              ...
              className={`p-4 bg-white rounded-xs shadow-md border-2 font-plus-jakarta-sans text-sm lg:text-base ${player.participantsRoleInTeam === '' ? 'text-gray-400' : 'text-black'} ${
                errors[index]?.participantsRoleInTeam ? 'border-red-500' : 'border-gray-200'
              }`}>
          ...
          <Field label={`LINK TWIBBON PEMAIN ${index + 1}`} error={errors[index]?.participantsTwibbon}>
            <input
              ...
              className={`p-4 bg-white rounded-xs shadow-md border-2 ${errors[index]?.participantsTwibbon ? 'border-red-500' : 'border-gray-200'}`}
          ...
          <Field label={`NOMOR WHATSAPP PEMAIN ${index + 1}`} error={errors[index]?.participantsPhone}>
            <input
              ...
              className={`p-4 bg-white rounded-xs shadow-md border-2 ${errors[index]?.participantsPhone ? 'border-red-500' : 'border-gray-200'}`}
...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1">
      <label className="text-sm lg:text-base font-fira-code uppercase">{label}</label>
      {children}
