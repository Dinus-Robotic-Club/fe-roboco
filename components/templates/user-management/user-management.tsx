...
const AddUserForm = ({ onSubmit, isPending, onClose }: { onSubmit: (data: IRegisterUserInput) => void; isPending?: boolean; onClose: () => void }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 p-6">
      ...
      <div className="space-y-2">
        ...
        <input type="text" required value={formData.name} onChange={(e) => setFormData((prev) => ({ ...prev, name: e.target.value }))} placeholder="Masukkan nama lengkap" className={inputClasses} />
      </div>
      ...
      <div className="space-y-2">
        ...
        <input type="email" required value={formData.email} onChange={(e) => setFormData((prev) => ({ ...prev, email: e.target.value }))} placeholder="contoh@email.com" className={inputClasses} />
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          />
          <button type="button" onClick={() => setShowPassword((prev) => !prev)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors">
          ...
          {(Object.keys(roleLabels) as UserRole[]).map((role) => (
            <button
              ...
              <div className="flex items-center justify-center gap-2">
                <div className={`w-3 h-3 rounded-full border-2 transition-all ${formData.role === role ? 'border-[#FBFF00] bg-[#FBFF00]' : 'border-slate-300'}`} />
                {roleLabels[role]}
              </div>
            </button>
          ))}
      ...
      <div className="flex gap-3 pt-4">
        <button type="button" onClick={onClose} className="flex-1 px-4 py-3 rounded-xl border border-slate-200 text-slate-600 font-medium hover:bg-slate-50 transition-colors">
          Batal
        </button>
        <button
          type="submit"
          disabled={isPending}
          className="flex-1 px-4 py-3 rounded-xl bg-slate-900 text-white font-medium hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2">
          {isPending ? (
            <>
...
// Main Component
export function UserManagement({ users = [], onAddUser, isPending }: UserManagementProps) {
  ...
  const handleAddUser = (data: IRegisterUserInput) => {
    onAddUser?.(data)
    ...
    <div className="w-full max-w-7xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {(Object.keys(roleLabels) as UserRole[]).map((role) => {
          ...
          return (
            <div key={role} className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-3">
      ...
      {/* User Table */}
      <GenericRankTable data={filteredUsers} columns={userColumns} subtitle={`${filteredUsers.length} users`} />
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-md bg-white p-0 gap-0 overflow-hidden rounded-2xl border-slate-200">
          <DialogHeader className="px-6 py-5 border-b border-slate-100 bg-slate-50/50">
            ...
            <DialogDescription className="text-slate-500">Isi detail informasi user di bawah ini.</DialogDescription>
          </DialogHeader>

          <AddUserForm onSubmit={handleAddUser} isPending={isPending} onClose={() => setIsDialogOpen(false)} />
        </DialogContent>
      </Dialog>
    </div>
  )
