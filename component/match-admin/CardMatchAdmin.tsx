...
export default function CardMatchAdmin({ data }: { data: ICardMatch }) {
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-6 lg:grid lg:grid-cols-5 items-start lg:items-center lg:w-full lg:max-w-6x md:px-3">
            <div className="flex flex-row-reverse lg:flex-row items-center gap-4 justify-end col-span-2">
              ...
              <div className="bg-logo-team w-[60px] h-[60px] sm:w-[70px] sm:h-[70px] flex items-center justify-center">
                <Image src={`${process.env.NEXT_PUBLIC_API_URL}${data?.teamA.logo}`} alt={data?.teamA.name} width={30} height={30} className="w-auto h-auto p-2" />
              </div>
            ...
            <div className="flex items-center gap-4 justify-start col-span-2 ">
              ...
              <div className="bg-logo-team w-[60px] h-[60px] sm:w-[70px] sm:h-[70px] flex items-center justify-center">
                <Image src={`${process.env.NEXT_PUBLIC_API_URL}${data?.teamB.logo}`} alt={data?.teamB.name} width={30} height={30} className="w-auto h-auto p-2" />
              </div>
      ...
      {showModalStart && (
        <ValidationModal setShowModalStart={setShowModalStart} action={goToMatch} title="Masuk ke Match?" desc="Apakah kamu ingin membuka halaman match ini?" confirm_text="Ya, lanjut" />
      )}
    </>
  )
}
