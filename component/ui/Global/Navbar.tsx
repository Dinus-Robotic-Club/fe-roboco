...
function Navbar({ left, right }: NavData) {
  ...
  const elemen_cta = ['Login', 'Logout', 'Dashboard']
  ...
  const modifiedRight = right
    ...
    .filter(Boolean) as typeof right
    ...
    <nav
      ...
      <div className="hidden md:flex gap-4 lg:gap-7 text-sm 2xl:text-lg w-full justify-end items-center">
        {left.map((item) => {
          ...
          return (
            <a key={item.title} href={item.href} onClick={(e) => handleNavClick(e, item.href)} className="relative font-semibold text-black transition-all duration-300 group scroll-smooth">
              {item.title}
              <span className={`absolute left-0 -bottom-1 h-[3px] bg-[#fcff00] rounded-full transition-all duration-300 ${active ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
            </a>
          )
        })}
      </div>
          ...
          return (
            <a
              ...
              className={`relative font-semibold text-black transition-all duration-300 group scroll-smooth ${
                elemen_cta.includes(item.title) ? 'bg-[#fcff00] text-black border-[1.5px] border-black px-8 py-2 rounded-full shadow hover:bg-[#FDFF9F]' : 'text-black group'
              }`}>
              {item.title}
      ...
      <div
        className={`fixed top-0 right-0 transition-all duration-300 w-[75%] sm:w-[60%] h-screen bg-white shadow-2xl z-50 flex flex-col p-6 gap-6 ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          {[...left, ...modifiedRight].map((item) => {
            ...
            return (
              <a
                ...
                href={item.href}
                onClick={(e) => {
                  handleNavClick(e, item.href, item.title)
                  setMenuOpen(false)
                }}
                className={`relative font-semibold text-black transition-all duration-300 group scroll-smooth ${
                  elemen_cta.includes(item.title) ? 'bg-[#fcff00] text-black border-[1.5px] border-black px-8 py-2 rounded-full shadow text-center hover:bg-[#FDFF9F]' : 'text-black group'
                }`}>
                {item.title}
                {elemen_cta.includes(item.title) ? (
                  ''
