...
export const TeamSchema = z
  .object({
    name: z
      ...
      .regex(/^[a-zA-Z0-9\s\-_]+$/, 'Nama team hanya boleh huruf, angka, spasi, strip, dan underscore'), // Anti XSS simpel

    email: z.string().trim().min(1, 'Email wajib diisi').email('Format email tidak valid (contoh: team@email.com)').toLowerCase(), // Sanitize input jadi lowercase
    ...
    communityName: z.string().trim().min(1, 'Asal sekolah/komunitas wajib diisi').min(3, 'Nama komunitas terlalu pendek').max(100, 'Nama komunitas terlalu panjang'),

    category: z
      .string()
      .trim()
      .min(1, 'Kategori lomba wajib dipilih')
      .refine((val) => ['SOCCER', 'SUMO'].includes(val), 'Kategori tidak valid'), // Hardlock value yang diperbolehkan

    tournamentId: z.string().trim().min(1, 'Turnamen wajib dipilih').uuid('ID Turnamen tidak valid'), // Asumsi ID dari DB adalah UUID
...
export const ParticipantSchema = z.object({
  participantsName: z
    ...
    .regex(/^[a-zA-Z\s'.]+$/, "Nama hanya boleh mengandung huruf, spasi, petik ('), dan titik (.)"), // Mencegah angka/simbol aneh di nama orang
  ...
  participantsTwibbon: z.string().trim().min(1, 'Link twibbon wajib diisi').url('Link twibbon harus berupa URL valid (http/https)').startsWith('https://', 'Link harus secure (https)'),
...
export const RegisterTeamSchema = z.object({
  team: TeamSchema,
  participants: z.array(ParticipantSchema).min(1, 'Minimal harus ada 1 pemain dalam tim').max(5, 'Maksimal 5 pemain dalam satu tim'), // Cegah spam user nambah array manual via console
})
